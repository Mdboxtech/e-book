...
@section('bdycontent')
    ...
    <div style="position: relative; margin-top:100px !important;" class="container">
        ...
        <section style="background:transparent; padding-top:0" class="contact-clean">
            <form id="mdupload" action="/uploadvideo" method="post" enctype="multipart/form-data">
                @csrf
                @if (session('success'))
                    <p class="text-success">{{ session('success') }}</p>
                @endif
                <h2 class="text-center">Upload video <i class="fa fa-video"></i></h2>
                <div class="mb-3">
                    <label for=""><b>video title</b></label>
                    <input class="form-control" type="text" name="videoTitle" placeholder="video title">
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control" type="file" name="videopath" placeholder="video" > 
                    @error('videopath')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
